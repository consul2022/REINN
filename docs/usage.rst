...
.. code-block:: bash
    ...
    $ reinn --help
    Usage: reinn [OPTIONS] COMMAND [ARGS]...
    ...
    Commands:
      serve  reinn CLI serve command.
    $ reinn serve --help
    Usage: reinn serve [OPTIONS]
...
.. code-block:: bash
    ...
    $ make image
    $ podman run -dit --name reinn -p 8000:8000 reinn:$(cat TAG)
    f41e5fa7ffd512aea8f1aad1c12157bf1e66f961aeb707f51993e9ac343f7a4b
    $ podman ps
    CONTAINER ID  IMAGE                                 COMMAND               CREATED        STATUS            PORTS                   NAMES
    f41e5fa7ffd5  localhost/reinn:0.1.0  /usr/bin/fastapi ...  2 seconds ago  Up 3 seconds ago  0.0.0.0:8000->8000/tcp  reinn
    $ curl localhost:8000/api/ready
    {"status":"ok"}
...
You can implement your own web routes logic straight away in ``reinn.controllers`` submodule. For more information please see `FastAPI documentation <https://fastapi.tiangolo.com/tutorial/>`__.
...
.. code-block:: bash
    ...
    Usage:
      ...
      clean-image      Clean reinn image
      install          Install reinn with poetry
      metrics          Run reinn metrics checks
      unit-test        Run reinn unit tests
      integration-test  Run reinn integration tests
      docs             Build reinn documentation
...
* RedisClient ``reinn.app.utils.redis``
* AiohttpClient ``reinn.app.utils.aiohttp_client``
...
.. code-block:: python
    ...
    from reinn.app.utils import RedisClient
...
.. literalinclude:: ../reinn/app/exceptions/http.py
    :language: python
...
All application configuration is available in ``reinn.config`` submodule.
...
.. literalinclude:: ../reinn/config/gunicorn.py
    :language: python

Routes
~~~~~~

Endpoints are defined in ``reinn.app.router`` submodule. Just simply import your controller and include it to FastAPI router:

.. literalinclude:: ../reinn/app/router.py
    :language: python
