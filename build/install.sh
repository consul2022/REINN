...
Now you should access CLI script: $ reinn --help
Alternatively you can access CLI script via poetry run: $ poetry run reinn --help
To deactivate virtualenv simply type: $ deactivate
To activate shell completion:
 - for bash: $ echo 'eval "$(_REINN_COMPLETE=source_bash reinn)' >> ~/.bashrc
 - for zsh: $ echo 'eval "$(_REINN_COMPLETE=source_zsh reinn)' >> ~/.zshrc
 - for fish: $ echo 'eval "$(_REINN_COMPLETE=source_fish reinn)' >> ~/.config/fish/completions/reinn.fish
EOF