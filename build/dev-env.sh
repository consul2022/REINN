...
minikube image load "reinn:${TAG}"

echo "[dev-env] creating reinn namespace"
kubectl create namespace reinn

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl get namespace reinn -n reinn"
...
kubectl create -f manifests/persistent-storage-no-pvc-deletion.yaml -n reinn

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment.apps/rfs-redisfailover-persistent-keep -n reinn"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
STATEFULSET_REPLICAS=$(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.spec.replicas}' -n reinn)
ATTEMPTS=0
until [[ ${STATEFULSET_REPLICAS} -eq $(kubectl get statefulset rfr-redisfailover-persistent-keep -o jsonpath='{.status.readyReplicas}' -n reinn) ]] || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done

echo "[dev-env] installing reinn charts"
helm upgrade --install \
    reinn charts/reinn \
    --namespace reinn \
    --set ingress.host.name="reinn.${INGRESS_HOST}"

ATTEMPTS=0
ROLLOUT_STATUS_CMD="kubectl rollout status deployment/reinn -n reinn"
until $ROLLOUT_STATUS_CMD || [ $ATTEMPTS -eq 60 ]; do
  ATTEMPTS=$((ATTEMPTS + 1))
  sleep 10
done
...
FastAPI available under: http://reinn.${INGRESS_HOST}/
